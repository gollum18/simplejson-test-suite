...
class TestDecoder(TestCase):
	...
	def test_json_object_correct(self):
		...
		"""
		test_input = ('{"abc": 0, "def": 1, "ghi": 2]', 0)
		dcdr = decoder.JSONDecoder()
		self.assertRaises(
			errors.JSONDecoderError,
			decoder.JSONObject,
			state=test_input,
			encoding=dcdr.encoding
			strict=dcdr.strict
	...
	def test_json_array_correct(self):
		"""
		Description: Tests that the JSONArrau method can decode
	...
	def test_json_decoder_create_utf(self):
		...
		self.assertEquals(dcdr.encoding, "utf-8")
	...
	def test_json_decoder_create_unicode(self):
		...
		"""
		dcdr = decoder.JSONDecoder(encoder="unicode")
		self.assertEquals(dcdr.encoding, "unicode")
	...
	def test_json_decoder_decode_correct(self):
		...
		dcdr = decoder.JSONDecoder()
		self.assertEquals(dcdr.decode(test_input), test_output)
	...
	def test_raw_decoder_decode_correct(self):
		...
		test_output = ["abc", "def", "ghi"]
		self.assertEquals(dcdr.raw_decode(test_input), test_output)
