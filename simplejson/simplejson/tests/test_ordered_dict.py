...
class TestOrderedDict(TestCase):
	...
	def test_clear(self):
		...
		"""
		od = TestOrderedDict.get_test_dict()
		self.assertNotEquals(len(od), 0)
		od.clear()
		self.assertEquals(len(od), 0)
	...
	def test_set_item(self):
		...
		self.assertNotIn(od.keys(), "test-0070")
		self.assertNotIn(od.values(), -1)
		od["test-0070"] = -1 
		self.assertIn(od.keys(), "test-0070")
		self.assertIn(od.values(), -1)
	...
	def test_iter(self):
		...
		key-value pairs from an ordered dictionary containing
		...
		Output:
			...
			key-value pairs in the following order:
			("abc", 0), ("def", 1), ("ghi", 2).

		Test Case: Corresponds to test case TEST-0072.
		"""
		test_output = [("abc", 0), ("def", 1), ("ghi", 2)]
		od = TestOrderedDict.get_test_dict()
		index = 0
		for keys, value in od.iter():
			k, v = *test_output[index]
	...
	def test_reversed(self):
		...
		magic method returns a generator function that yields
		key-value pairs in reverse order from an ordered
		dictionary containing the following mappings:
		{"abc": 0, "def": 1, "ghi": 2}.
		...
		"""
		test_output = [("ghi", 2), ("def", 1), ("abc", 0)]
		od = TestOrderedDict.get_test_dict()
		index = 0
		for keys, value in od.reversed():
			k, v = *test_output[index]
			self.assertEquals(k, key)
			self.assertEquals(v, value)
			index += 1
	...
	def test_pop_item_rear(self):
		...
		test_output = ("ghi", 2)
		od = TestOrderedDict.get_test_dict()
		self.assertEquals(od.popitem(True), test_output)
	...
	def test_pop_item_front(self):
		...
		test_output = ("abc", 0)
		od = TestOrderedDict.get_test_dict()
		self.assertEquals(od.popitem(False), test_output)
	...
	def test_keys(self):
		...
		index = 0
		for key in od.keys():
			self.assertEquals(key, test_output[index])
			index += 1
	...
	def test_fromkeys_correct(self):
		...
		index = 0
		for key in od.keys():
			self.assertEquals(key, test_input[index])
			index += 1
	...
	def test_equals_true(self):
		...
		self.assertEquals(od.__eq__(other), True)
	...
	def test_equals_false(self):
		...
		self.assertEquals(od.__eq__(other), False)
	...
	def test_not_equals_false(self):
		...
		od = TestOrderedDict.get_test_dict()
		other = TestOrderedDict.get_test_dict()
		self.assertEquals(od.__ne__(other), False)
	...
	def test_not_equals_true(self):
		...
		od = TestOrderedDict.get_test_dict()
		other = ordered_dict.OrderedDict()
		self.assertEquals(od.__ne__(other), True)
